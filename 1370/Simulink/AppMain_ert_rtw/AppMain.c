...
void AppMain_step(void)
{
  ...
  MotorRestartAndFaultCnt(rtb_MotorTargetSpeed, rtb_ECUFault, U.RestartComplete,
    ...
    &rtb_Switch_jqwg, &Y.MotorRestartEnable, &Y.ShortCnt, &Y.OpenCnt, &Y.LockCnt,
    &B.MotorRestartAndFaultCnt_bnwn, &DW.MotorRestartAndFaultCnt_bnwn);
  ...
  /* Chart: 'AmbTPreOverDiag' (':3119') incorporates:
   *  Inport: 'AmbTemperature' (':767')
   *  Outport: 'AmbTPreOverStatus' (':3101')
   */
  TemperaturState(U.AmbTemperature, &Y.AmbTPreOverStatus, 100, 140, 138, 2000,
                  -40, -38, &DW.sf_AmbTPreOverDiag);

  /* Switch: 'Switch' (':2337') incorporates:
   *  Outport: 'AmbTPreOverStatus' (':3101')
   */
  if ((int32_T)Y.AmbTPreOverStatus > 0)
  {
     ...
     */
    if (3277 < (int32_T)rtb_Switch_jqwg)
    {
    ...
    {
      /* Outport: 'TargetSpeedLimited' (':2216') */
      Y.TargetSpeedLimited = rtb_Switch_jqwg;
    }
...
void AppMain_initialize(void)
{
  ...
  MotorRestartAndFaultCnt_Init(&B.MotorRestartAndFaultCnt_bnwn,
    &DW.MotorRestartAndFaultCnt_bnwn);
  ...
  /* SystemInitialize for Chart: 'AmbTPreOverDiag' (':3119') incorporates:
   *  Outport: 'AmbTPreOverStatus' (':3101')
   */
  TemperaturState_Init(&Y.AmbTPreOverStatus, &DW.sf_AmbTPreOverDiag);
