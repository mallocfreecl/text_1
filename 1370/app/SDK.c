...
void SDK_ADCGetChResult(u8 Ch, u16* pResult)
{
  ...
  switch(Ch>>4u)
  {
    ...
    case 2u:
      ...
      break;
    default:/*消除polyspace警告,2021-09-22-by zyg*/
    break;
  }
...
{
  /* TLE987x只有一个DFlash Sector,所以只能擦除这个Sector，22.06.03 by lxs */
  GPT12E_T2_Stop();
...
void SDK_BDRVShortDiag(BDRVOffDiagResult_t * DiagResult)
{
  ...
  iBDRV_Int_En = CPU->NVIC_ISER0.bit.Int_BDRV;//中断使能
  ...
  BDRV_Set_Bridge(Ch_En, Ch_En, Ch_En, Ch_En, Ch_En, Ch_En);//通道使能

  /* set IDISCHARGE = 1, set DSMONVTH = 0 (0.25V) */
  BDRV_Set_DSM_Threshold(Threshold_0_25_V);
  BDRV->CTRL3.bit.IDISCHARGEDIV2_N = 0u;//半量程
