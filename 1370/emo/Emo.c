...
#define I_SHUNT_LIMIT_MIN (102)
#define I_SHUNT_LIMIT_MAX (104)
#define CURRENT_LIMIT_STEP_TIME (50u)
#define CURRENT_LIMIT_STEP_TIME_OVER (2u)
#define CURRENT_LIMIT_POS_INC_STEP (25)
#define CURRENT_LIMIT_POS_DEC_STEP (1)/*1*/
#define CURRENT_LIMIT_NEG_INC_STEP (5)
#define CURRENT_LIMIT_NEG_DEC_STEP (20)/*20*/
#define CURRENT_LIMIT_DIFF (10)/*unit:0.1A*/
...
static void Emo_calIShuntZero(void)
{
  ...
  if (Err != 0u)
  {
    Emo_CSA_Offset = DTCVar.IShuntZero;
  }

  ADCResults.ErrFlags.IShuntZero =
    (Emo_CSA_Offset > EMO_ISHUNT_ZERO_MAX) || (Emo_CSA_Offset < EMO_ISHUNT_ZERO_MIN);	
...
void Emo_MLowTask(void)
{
  ...
  {			
    ...
    Is = Mat_ExePi(&Emo_Ctrl.SpeedPi, Emo_Ctrl.RefSpeed - Emo_Ctrl.ActSpeed);
		
		if((MTPA.En) == 1u)
		{
			#if 0
			Emo_Ctrl.DemagCurr = MTPA.K - sqrt(MTPA.K*MTPA.K + Is*Is/2);
			Emo_Ctrl.RefCurr = sqrt(Is*Is - Emo_Ctrl.DemagCurr*Emo_Ctrl.DemagCurr);
			#endif
			sint32 Cos;
			sint32 Sin;
			
			/* Get angle functions */
			Cos = pTable_Cos[MTPA.K];
			Sin = Table_Sin[MTPA.K];
			Emo_Ctrl.DemagCurr = ABS((sint16)__ssat(Mat_FixMul(Is, Cos), MAT_FIX_SAT));
			Emo_Ctrl.RefCurr = (sint16)__ssat(Mat_FixMul(Is, Sin), MAT_FIX_SAT);
		}
		else
		{
			//Emo_Ctrl.DemagCurr = 0;
